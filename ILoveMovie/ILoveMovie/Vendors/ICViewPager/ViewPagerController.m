...
- (void)defaultSettings {
    ...
    if ([self.delegate respondsToSelector:@selector(viewPagerwidth)]){
        _tabWidth = [self.delegate viewPagerwidth];
    }
...
- (void)reloadData {
    ...
    if ([self.delegate respondsToSelector:@selector(viewPager:valueForOption:withDefault:)]) {
        ...
        if ([self.delegate respondsToSelector:@selector(viewPagerwidth)]){
             _tabWidth = [self.delegate viewPager:self valueForOption:ViewPagerOptionTabWidth withDefault:[self.delegate viewPagerwidth]];
        }
...
- (void)pageViewController:(UIPageViewController *)pageViewController willTransitionToViewControllers:(NSArray *)pendingViewControllers {
   // NSLog(@"willTransitionToViewController: %lu", (unsigned long)[self indexForViewController:[pendingViewControllers objectAtIndex:0]]);
}
