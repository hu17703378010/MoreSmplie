...
#define kPW [UIScreen mainScreen].bounds.size.width/375
#define kPH [UIScreen mainScreen].bounds.size.height/667
...
- (id)initWithPoint:(CGPoint)point radius:(int)radiusValue inView:(UIView *)inView
{
    ...
    // 动画的
    self = [super initWithFrame:CGRectMake(point.x + 180*kPW, point.y + 60*kPH, 2 * radiusValue, 2 * radiusValue)];
    if (self) {
        self.radius = radiusValue;
        self.bubbleRadius = 40;
        self.parentView = inView;
        
        self.facebookBackgroundColorRGB = 0xd35b6a;
       //  self.twitterBackgroundColorRGB = 0x3083be;
//        self.mailBackgroundColorRGB = 0xbb54b5;
        self.googlePlusBackgroundColorRGB = 0xd35b6a;
        self.tumblrBackgroundColorRGB = 0xd35b6a;
    }
    return self;
}
...
-(void)show
{
    if(!self.isAnimating)
    {
        ...
        if(self.showFacebookBubble) {
            UIButton *facebookBubble = [self shareButtonWithIcon:@"weibo123.png" andBackgroundColorRGB:self.facebookBackgroundColorRGB];
        ...
        if(self.showTwitterBubble) {
            UIButton *twitterBubble = [self shareButtonWithIcon:@"tengxun123.png" andBackgroundColorRGB:self.twitterBackgroundColorRGB];
        ...
        if(self.showGooglePlusBubble) {
            UIButton *googlePlusBubble = [self shareButtonWithIcon:@"douban123.png" andBackgroundColorRGB:self.googlePlusBackgroundColorRGB];
        ...
        if(self.showTumblrBubble) {
            UIButton *googlePlusBubble = [self shareButtonWithIcon:@"renren123.png" andBackgroundColorRGB:self.tumblrBackgroundColorRGB];
